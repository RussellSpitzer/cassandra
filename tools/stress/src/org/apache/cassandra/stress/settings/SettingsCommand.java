...
public abstract class SettingsCommand implements Serializable
{
    ...
    public final RetryBackoff backoff;
    ...
    public SettingsCommand(Command type, Options options, Count count, Uncertainty uncertainty)
    {
        ...
        this.backoff = new RetryBackoff(options.backoffStrategy.value(),
                Integer.parseInt(options.backoffSeconds.value()));
    ...
    static abstract class Options extends GroupedOptions
    {
        ...
        final OptionSimple backoffStrategy = new OptionSimple("backoff_strategy=","CONSTANT|LINEAR|EXPONENTIAL","CONSTANT", "Backoff strategy in case of driver exception {CONSTANT,LINEAR,EXPONENTIAL}", false);
        final OptionSimple backoffSeconds = new OptionSimple("backoff_seconds=","[0-9]+","1", "Coeffecient for backoff strategy in seconds", false);
    ...
    static class Count extends Options
    {
        ...
        {
            return Arrays.asList(count, retries, backoffStrategy, backoffSeconds, noWarmup, ignoreErrors, consistencyLevel, atOnce);
        }
    }
    ...
    static class Uncertainty extends Options
    {
        ...
        public List<? extends Option> options()
        {
            return Arrays.asList(uncertainty, minMeasurements, maxMeasurements, retries, backoffStrategy,
                    backoffSeconds, noWarmup, ignoreErrors, consistencyLevel, atOnce);
        }
    }
